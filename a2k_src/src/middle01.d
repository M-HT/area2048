...
void	TSKmiddle01(int id)
{
	...
	switch(TskBuf[id].step){
		case	0:
			...
			TskBuf[id].fp_draw = &TSKmiddle01Draw;
			TskBuf[id].fp_exit = &TSKmiddle01Exit;
		...
		case	1:
			...
			if(!TskBuf[id].wait){
				TskBuf[id].fp_int = &TSKmiddle01Int;
...
void	TSKmiddle01Draw(int id)
{
	...
	foreach(k; 0..5){
		int	bodyNumVertices = (k == 0)?4:3;
		int	startOffset = (k == 0)?0:(4+3*(k-1));
