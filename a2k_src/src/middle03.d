...
void	TSKmiddle03(int id)
{
	...
	switch(TskBuf[id].step){
		case	0:
			...
			TskBuf[id].fp_draw = &TSKmiddle03Draw;
			TskBuf[id].fp_exit = &TSKmiddle03Exit;
		...
		case	1:
			...
			if(!TskBuf[id].wait){
				TskBuf[id].fp_int = &TSKmiddle03Int;
		...
		case	3:
			...
			for(int i = 0; i < 4; i++){
				effSetBrokenBody(id, middle_poly, (4 + i * 4), 4,+0.0f,+0.0f);
				effSetBrokenLine(id, middle_poly, (4 + i * 4), 4,+0.0f,+0.0f);
			}
			TskBuf[id].step = -1;
			break;
		default:
