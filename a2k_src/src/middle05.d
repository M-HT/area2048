...
void	TSKmiddle05(int id)
{
	...
	switch(TskBuf[id].step){
		case	0:
			...
			TskBuf[id].fp_draw = &TSKmiddle05Draw;
			TskBuf[id].fp_exit = &TSKmiddle05Exit;
		...
		case	1:
			...
			if(!TskBuf[id].wait){
				TskBuf[id].fp_int = &TSKmiddle05Int;
		...
		case	3:
			...
			for(int i = 0; i < 4; i++){
				effSetBrokenBody(id, middle_poly, (4 + i * 8), 8,+0.0f,+0.0f);
				effSetBrokenLine(id, middle_poly, (4 + i * 8), 8,+0.0f,+0.0f);
			}
			TskBuf[id].step = -1;
			break;
		default:
